...
type WalletQuery struct {
	...
	withWithdrawals  *WithdrawalRequestQuery
...
func (_q *WalletQuery) QueryWithdrawals() *WithdrawalRequestQuery {
	query := (&WithdrawalRequestClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *WalletQuery) WithWithdrawals(opts ...func(*WithdrawalRequestQuery)) *WalletQuery {
	query := (&WithdrawalRequestClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *WalletQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Wallet, error) {
	...
	if query := _q.withWithdrawals; query != nil {
		if err := _q.loadWithdrawals(ctx, query, nodes,
			func(n *Wallet) { n.Edges.Withdrawals = []*WithdrawalRequest{} },
			func(n *Wallet, e *WithdrawalRequest) { n.Edges.Withdrawals = append(n.Edges.Withdrawals, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *WalletQuery) loadWithdrawals(ctx context.Context, query *WithdrawalRequestQuery, nodes []*Wallet, init func(*Wallet), assign func(*Wallet, *WithdrawalRequest)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Wallet)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(withdrawalrequest.FieldWalletID)
	}
	query.Where(predicate.WithdrawalRequest(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(wallet.WithdrawalsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
